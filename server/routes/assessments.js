  ...
  userDb
    ...
    .then(assmts => {
      let queries = assmts.map(obj => {
        return subDb.getSubmissionByRecordId(obj.assessment_record).then(subs => {
          obj.evidence = subs
          return obj
        })
      })
...
router.post("/submission", decode, (req, res) => {
  ...
  userDb
    ...
    .then(user_id => {
      return db
        .getUserAssessment(user_id, code)
    ...
    .then(student_assessment_id => {
      ...
      return db.saveSubmission(sub).then(() => res.json({ record_id: student_assessment_id }))
    })
    .catch(err => res.status(500).json({ err: "Server Error", message: err.message }))
})
